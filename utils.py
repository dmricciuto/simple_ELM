...
def daily_to_monthly(var_in, allyearsmean=False):
    ...
    ndays = max(var_in.shape)
    if (allyearsmean):
        nmonths = 12
    else:
        nmonths = int(ndays / 365) * 12
    #get number of variables to process
    nvar=1
    if (var_in.ndim > 1):
      nvar = min(var_in.shape)

    doy_in = (numpy.cumsum(numpy.ones([ndays], numpy.float)) - 1) % 365 + 1
    if var_in.ndim == 1:
      var_out = numpy.zeros([nmonths], numpy.float)
    else:
      var_out = numpy.zeros([nvar,nmonths], numpy.float)

    for dim in range(0,nvar):
      var_out_temp = numpy.zeros([nmonths], numpy.float)
      if (var_in.ndim > 1):
        var_in_temp = var_in[dim,:].squeeze()
      else:
        var_in_temp = var_in
      for d in range(0, ndays):
        ...
        if (not allyearsmean):
            ...
            var_out_temp[thismonth_out] = var_out_temp[thismonth_out] + \
                var_in_temp[d] / (ndays_month[thismonth])
        else:
            var_out_temp[thismonth] = var_out_temp[thismonth] + \
                var_in_temp[d] / (ndays_month[thismonth] * ndays / 365)
        if (var_in.ndim == 1):
          var_out = var_out_temp
        else:
          var_out[dim,:] = var_out_temp
